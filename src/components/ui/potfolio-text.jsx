...
export function AnimatedLetterText({ text = "Portfolio", letterToReplace = "o", className }) {
  const lowerText = text.toLowerCase()
  const lowerLetter = letterToReplace.toLowerCase()
  const replaceIndex = lowerText.indexOf(lowerLetter)

  if (replaceIndex === -1) {
    return <span className={cn("font-black tracking-tight", className)}>{text}</span>
  }
    ...
    <span className={cn("inline-flex items-center font-black tracking-tight", className)}>
      ...
      >
        ...
        <span className="absolute inset-0 flex items-center justify-center">
          <svg
            viewBox="0 0 100 100"
            className="w-[0.32em] h-[0.32em] animate-spin"
